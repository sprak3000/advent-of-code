...
// Replacements required by part 1
$input[1] = 12;
$input[2] = 2;
...
do {
    $input1 = $input[$input[$index + 1]];
    $input2 = $input[$input[$index + 2]];
    $storeIndex = $input[$index + 3];

    switch ($input[$index]) {
        case $addOpCode:
            $input[$storeIndex] = $input1 + $input2;
            break;
        case $multiplyOpCode:
            $input[$storeIndex] = $input1 * $input2;
            break;
        case $haltOpCode:
            ...
            break;
        default:
            exit('Something went wrong. Opcode: ' . $input[$index]);
    }

    $index += 4;
} while (!$halt);

echo 'Part 1 Result: ' . $input[0] . "\n";

/*
--- Part Two ---

"Good, the new computer seems to be working correctly! Keep it nearby during this mission - you'll probably use it again.
Real Intcode computers support many more features than your new one, but we'll let you know what they are as you need
them."
