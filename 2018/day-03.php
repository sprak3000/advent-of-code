...
echo 'Part 1 Answer: Overlaps in Claims = ' . $overlaps . "\n";
...
foreach ($input as $index => $claim) {
    ...
    for ($i = $index + 1; $i < $totalClaims; $i++) {
        $claimDetails = [];
        preg_match('/#(\d+) @ (\d+),(\d+): (\d+)x(\d+)/', $input[$i], $claimDetails);
...
$noOverlapClaimId = array_shift(array_diff($allClaimIds, $overlappedClaimIds));

echo 'Part 2 Answer: No Overlap ID = ' . $noOverlapClaimId . "\n";
