...
foreach ($input as $passphrase) {
    $counts = array_count_values($passphrase);
...
// count_chars
foreach ($input as $passphrase) {
    array_walk($passphrase, function (&$pItem, $pIndex) {
        $characters = str_split($pItem);
        sort($characters);
        $pItem = implode('', $characters);
    });

    if (1 === max(array_count_values($passphrase))) {
        $sum++;
    }
}
