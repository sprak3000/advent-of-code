...
while (!$exit) {
    ...
    // Will this step lead us out?
    if ($boundary < $currentIndex) {
        $exit = true;
        break;
    }

    // If we are moving, update to that index
    if (0 !== $input[$currentIndex]) {
        $currentIndex += $input[$currentIndex];
    }

    // Update our current position's value
    $input[$previousIndex]++;
...
echo 'Part 1 Answer: Steps = ' . $steps . "\n";

/*
-- Part Two ---

Now, the jumps are even stranger: after each jump, if the offset was three or more, instead decrease it by 1. Otherwise,
...
while (!$exit) {
    ...
    // If we are moving, update to that index
    if (0 !== $input2[$currentIndex]) {
        $currentIndex += $input2[$currentIndex];
    }

    // Update our current position's value
    $input2[$previousIndex] += (3 <= $input2[$previousIndex]) ? -1 : 1;

    // Take a step
    $steps++;
}

echo 'Part 2 Answer: Steps = ' . $steps . "\n";
